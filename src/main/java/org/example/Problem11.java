...
public class Problem11 {
    ...
    public static void main(String[] args) {
        ...
        int greatestProduct = Integer.MIN_VALUE;
        for (int i = 0; i < 20; i++) {
            for (int j = 0; j < 20; j++) {
                // Check the product to the right
                if (j <= 16) {
                ...
                // Check the product diagonal to the right and down
                if (i <= 16 && j <= 16) {
                ...
                // Check the product diagonal to the left and down
                if (i <= 16 && j >= 3) {
                    int product = grid[i][j] * grid[i + 1][j - 1] * grid[i + 2][j - 2] * grid[i + 3][j - 3];
                                //grid[i][j] * grid[i - 1][j - 1] * grid[i - 2][j - 2] * grid[i - 3][j - 3];
                    if (product > greatestProduct) {
